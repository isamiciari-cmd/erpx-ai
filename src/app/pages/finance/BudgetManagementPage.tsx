...
const budgets = [
  { id: 1, name: 'Sales Department Q2 2026', type: 'Department', allocated: 120000, used: 102000, committed: 8000, available: 10000, utilization: 85 },
  { id: 2, name: 'Marketing Campaign 2026', type: 'Project', allocated: 50000, used: 46000, committed: 2000, available: 2000, utilization: 92 },
  { id: 3, name: 'Operations Monthly', type: 'Department', allocated: 100000, used: 78000, committed: 12000, available: 10000, utilization: 78 },
  { id: 4, name: 'IT Infrastructure', type: 'Department', allocated: 60000, used: 39000, committed: 8000, available: 13000, utilization: 65 },
  { id: 5, name: 'HR & Recruitment', type: 'Department', allocated: 80000, used: 70400, committed: 4600, available: 5000, utilization: 88 },
];
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Plan, track, and analyze budgets across departments and projects</p>
        </div>
            ...
            <tbody className="divide-y">
              {budgets.map((budget) => (
                <tr key={budget.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-right">${budget.allocated.toLocaleString()}</td>
                  <td className="px-6 py-4 text-sm text-right text-purple-600 font-medium">${budget.used.toLocaleString()}</td>
                  <td className="px-6 py-4 text-sm text-right text-green-600 font-bold">${budget.available.toLocaleString()}</td>
                  <td className="px-6 py-4">
                    <div className="w-full bg-gray-200 rounded-full h-2 mb-1">
                      <div
                        className={`h-2 rounded-full ${
                          budget.utilization >= 90 ? 'bg-red-500' :
                          budget.utilization >= 75 ? 'bg-yellow-500' :
                          'bg-green-500'
                        }`}
