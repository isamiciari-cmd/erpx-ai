...
const recentOrders = [
  { id: "PO-1234", supplier: "Al-Jawad Trading", amount: 12500, status: "Received", date: "2h ago", grn: "GRN-456" },
  { id: "PO-1235", supplier: "Modern Supply", amount: 8900, status: "In Transit", date: "5h ago", grn: "-" },
  { id: "PO-1236", supplier: "Tech Imports", amount: 15200, status: "Approved", date: "1d ago", grn: "-" },
  { id: "PO-1237", supplier: "Global Wholesale", amount: 6700, status: "Delayed", date: "2d ago", grn: "-" },
];

// Purchase requests pending approval
const pendingRequests = [
  { id: "PR-245", department: "IT", item: "Office Supplies", quantity: 500, value: 5000, requestedBy: "Ahmad", priority: "High" },
  { id: "PR-246", department: "Operations", item: "Computer Parts", quantity: 50, value: 12000, requestedBy: "Sarah", priority: "Medium" },
  { id: "PR-247", department: "Marketing", item: "Furniture", quantity: 20, value: 8500, requestedBy: "Mohammed", priority: "Low" },
];
...
// Goods receipts
const recentGRN = [
  { id: "GRN-456", po: "PO-1234", supplier: "Al-Jawad", received: 48, ordered: 50, variance: -2, status: "Accepted" },
  { id: "GRN-457", po: "PO-1238", supplier: "Modern Supply", received: 100, ordered: 100, variance: 0, status: "Accepted" },
  { id: "GRN-458", po: "PO-1239", supplier: "Tech Imports", received: 28, ordered: 30, variance: -2, status: "Partial" },
];

// Three-way matching
const threeWayMatch = [
  { invoice: "INV-789", po: "PO-1234", grn: "GRN-456", poAmount: 12500, grnAmount: 12000, invoiceAmount: 12500, status: "Mismatch" },
  { invoice: "INV-790", po: "PO-1238", grn: "GRN-457", poAmount: 8900, grnAmount: 8900, invoiceAmount: 8900, status: "Matched" },
];
...
const aiInsights = [
  {
    ...
    message: "Switch to 'Modern Supply Co' for electronics - save 12% on average. Estimated annual savings: SAR 18,000.",
    confidence: "91%",
  },
  {
    id: 2,
    ...
    message: "'Global Wholesale' has 3 consecutive delays averaging 4 days. Consider backup supplier or contract renegotiation.",
    confidence: "87%",
  },
  {
    id: 3,
    ...
    message: "Seasonal discount from 'Al-Jawad Trading' - 15% off on orders >SAR 50K. Valid until month end.",
    confidence: "94%",
  },
  {
    id: 4,
    type: "warning",
    title: "Budget Alert",
    message: "IT department at 92% budget utilization. Recommend approval freeze for non-critical requests.",
    confidence: "96%",
  },
];
              ...
              <div key={req.id} className="p-4 hover:bg-gray-800/30 transition-colors">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{req.id} • {req.department}</p>
                  </div>
                  <span
        ...
        >
          ...
          <div className="space-y-4">
            {budgetData.map((dept) => {
              ...
              const availablePercent = ((dept.budget - dept.used - dept.committed) / dept.budget) * 100;
              ...
              <div key={supplier.id} className="p-4 hover:bg-gray-800/30 transition-colors">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400">{supplier.orders} orders • ⭐ {supplier.rating}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-white font-semibold text-sm">SAR {supplier.amount.toLocaleString()}</p>
                  </div>
                </div>
              ...
              <div key={match.invoice} className="p-4 hover:bg-gray-800/30 transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">PO: {match.po} • GRN: {match.grn}</p>
                  </div>
                  <span
                    className={`px-2 py-1 rounded text-xs font-semibold ${
                ...
                {match.status === "Mismatch" && (
                  <p className="mt-2 text-xs text-red-400">
                    ⚠️ GRN variance detected: SAR {Math.abs(match.poAmount - match.grnAmount).toLocaleString()}
                  </p>
                )}
              </div>
            ))}
              ...
              <div key={grn.id} className="p-4 hover:bg-gray-800/30 transition-colors">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{grn.id} • PO: {grn.po}</p>
                  </div>
                  <span
                    className={`px-2 py-1 rounded text-xs font-semibold ${
                      grn.status === "Accepted"
                ...
                <div className="flex items-center gap-4 text-xs">
                  ...
                  {grn.variance !== 0 && (
                    <div className={`px-2 py-1 rounded ${grn.variance < 0 ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
                      {grn.variance > 0 ? '+' : ''}{grn.variance}
                    </div>
                  )}
                </div>
              </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <span className={`text-sm font-semibold flex items-center gap-1 ${trendDown ? 'text-green-400' : 'text-green-400'}`}>
