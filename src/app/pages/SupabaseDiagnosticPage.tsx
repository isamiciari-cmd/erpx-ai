  ...
  const runConnectionTest = async (): Promise<boolean> => {
    ...
    if (isDemoMode) {
      logs = log(
        logs,
        'warn',
        'DEMO MODE active — VITE_SUPABASE_URL or VITE_SUPABASE_ANON_KEY is missing.',
      );
      patch('connection', {
        status: 'warn',
        detail:
          'DEMO MODE — Supabase credentials not configured. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to enable real DB access.',
        logs,
      });
      return false;
    }
        ...
        <div className="flex items-center gap-3 rounded-xl border border-emerald-500/50 bg-emerald-900/20 px-6 py-4">
          ...
          <div>
            ...
            <p className="text-emerald-400/70 text-sm">
              All tests passed. ERPX-AI is fully connected to Supabase PostgreSQL.
            </p>
          </div>
        </div>
      );
    ...
    <div className="min-h-screen bg-[#0A0E17] text-white p-6">
      ...
      <div className="space-y-4">
        {tests.map((test) => (
          <div
            key={test.id}
            className={`rounded-xl border p-4 transition-all ${statusBg(test.status)}`}
          >
